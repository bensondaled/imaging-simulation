#things generated recently

#may12: (working on batch6, thresh_match 0.2)
#run analyze.py figure 5
#np.savez('for_andrea',in_ca=in_ca,out_ca=out_ca,offsets=offsets,time_peak_cross=jitters,corr_coefs=corrs,exprs=exprs) 
...
class Result(object):
    ...
    def __init__(self, in_path, out_path):
        ...
        self.matches_all2 = self.match_cells2()
    ...
    def get_input_cell_summaries(self):
        ...
        for ci,c in enumerate(self.inn['cells']):
            ...
            cells[ci]['match2'][:] = self.matches_all2[ci]
        return cells
    ...
    def match_cells2(self):
        #after april 15 2015 discussion with andrea
        matches = []
        for ii,im in enumerate(self.in_masks):
            out_match_idx = np.argmax([overlap(im,om,percent_of=1) for om in self.out_masks])
            perc_out = overlap(im, self.out_masks[out_match_idx], percent_of=2)
            perc_in = overlap(im, self.out_masks[out_match_idx], percent_of=1)

            matches.append([ii,out_match_idx, perc_in, perc_out])
        return np.array(matches)
...
if __name__ == '__main__':
    ...
    if option == 'merge':
        ...
        if figidx == 1:
            ...
            in_ca = [normalize(i['ca'][1::2]) for i in inn_matched]
            out_ca = [normalize(i['C']) for i in out_matches]

            match_percs = [i['match2'][-2:] for i in inn_matched]
            offsets = [i['offset'] for i in inn_matched]    

            jitters = 0.128*(np.array([np.argmax(np.correlate(*pair,mode='full')) for pair in zip(in_ca,out_ca)])-(len(in_ca[0])-1))
            corrs = np.array([np.corrcoef(*pair)[0,1] for pair in zip(in_ca,out_ca)])
            exprs = [ii['expression'] for ii in inn_matched]
            #pl.hist(jitters)
